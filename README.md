...
基于上游 [aidenlab/Juicebox](https://github.com/aidenlab/juicebox) v2.17.00（commit `c7b6988`），在 Windows 上以 JDK 25 + Gradle 构建，针对装配编辑（JBAT 工作流）与热图渲染做了多轮性能优化。

版本对照：基线 `b17f0e8` → 优化主提交 `f8ea3fb` → 当前 `e69146f`（含模块化重构 + Gradle 迁移）。
...
src/juicebox/      桌面应用（juicebox-app 模块），通过 core 模块获取算法
```
